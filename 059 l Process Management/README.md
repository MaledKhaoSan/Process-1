...
คำสั่ง `ls` จะแสดงเนื้อหาในไดเรกทอรีปัจจุบัน ระบุรายการไฟล์และไดเรกทอรีทั้งหมด.
...
คำสั่ง `cd` จะเปลี่ยนไดเรกทอรีปัจจุบันเป็น "Documents."

### 3. `mkdir` - สร้างไดเรกทอรี

```bash
$ mkdir NewDirectory
```

คำสั่ง `mkdir` จะสร้างไดเรกทอรีใหม่ชื่อ "NewDirectory."

### 4. `grep` - ค้นหาข้อความในไฟล์
